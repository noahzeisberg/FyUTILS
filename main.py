...
def resolve_fuel_information(file):
    ...
    print(prefix("FUEL") + "FUEL command name: " + fuel_json["command_name"])
    print(prefix("FUEL") + "FUEL type: " + fuel_json["type"])
    fuel_command_list.append(fuel_json["command_name"])
...
while True:
    ...
    try:
        ...
        if os.getcwd() == current_dir:
            ...
            cwd_abbreviation = os.getcwd().replace("C:\\", "/").replace("\\", "/").replace(":", "")
    ...
    match cmd:
        ...
        case "fuel":
            ...
            if fuel_action == "install":
                ...
                if os.path.exists(fuel_content_dir + fuel_file):
                ...
                for i in range(len(fuel_repo_contents)):
                    fuel_download_url = ""
                    if fuel_repo_contents[i]["name"] == fuel_file:
                ...
                local_fuel_file = open(fuel_content_dir + fuel_file, "xb")
                local_fuel_file.write(fuel_file_content)
                local_fuel_file.close()
                local_fuel_file = open(fuel_content_dir + fuel_file, "r")
                print(prefix("FUEL") + "FUEL \"" + fuel_location + "\" successfuly installed to \"" + fuel_content_dir + "\".")
                print(prefix("FUEL") + "Adding FUEL to FyUTILS...")
                fuel_command_list.append(json.load(local_fuel_file)["command_name"])
                fuel_list.append(fuel_file)
                print(prefix("FUEL") + f"Done! Took{time.time() - activity_start: 0.2f}s to install package " + Fore.LIGHTMAGENTA_EX + fuel_location)

            elif fuel_action == "remove":
                fuel_file = fuel_location + ".json"
                print(prefix("FUEL") + "Unregistering " + fuel_content_dir + fuel_file + "...")
                if os.path.exists(fuel_content_dir + fuel_file):
                    fuel_command_list.remove(json.load(open(fuel_content_dir + fuel_file))["command_name"])
                    fuel_list.remove(fuel_file)
                else:
                    print(prefix("ERROR") + "Package \"" + fuel_content_dir + fuel_file + "\" remove failed!")
                    print(prefix("ERROR") + "Error: Local package not found.")
                    continue
                print(prefix("FUEL") + "Deleting " + fuel_content_dir + fuel_file + "...")
                if os.path.exists(fuel_content_dir + fuel_file):
                    os.remove(fuel_content_dir + fuel_file)
                else:
                    print(prefix("ERROR") + "Package \"" + fuel_content_dir + fuel_file + "\" remove failed!")
                    print(prefix("ERROR") + "Error: Local package not found.")
                    continue
        ...
        case "edit":
            ...
            for i in range(sys.maxsize):
                try:
                    edit_string = edit_string + input(str(i) + ": ") + "\n"
                except KeyboardInterrupt:
                    print("END OF FILE")
                    print("")
                    break
